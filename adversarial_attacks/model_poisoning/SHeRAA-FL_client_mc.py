...
#Description: Model Cancelling attack FL Client Program for ISCX-VPN 2016 (SHeRAA-FL client experiment)
...
class ntcClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        history = self.model.fit(self.x_train, self.y_train, epochs=config['training_model_epochs'], batch_size=config['training_model_batch_size'], shuffle = True)
        
        #Model cancelling attack (set layers weight to zero)
        wx = np.zeros([self.x_train.shape[1],6])
        bx = np.zeros(6)

        wx2 = np.zeros([6,6])
        bx2 = np.zeros(6)
        
        self.model.layers[0].set_weights([wx,bx])
        self.model.layers[1].set_weights([wx2,bx2])
